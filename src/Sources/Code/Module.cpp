...
#if _DEBUG
	fprintf(stderr, "MobiFlight[%s]: RegisterSimVar > %s ID [%u] : Offset(%u) : Value(%f, %d, %s)", client->Name.c_str(), var1.Name.c_str(), var1.ID, var1.Offset, var1.Value, ival, var1.StringValue.c_str());
#endif
}
...
#if _DEBUG
	fprintf(stderr, "MobiFlight[%s]: SimVar %s with ID %u has value %f, %d, %s", client->Name.c_str(), simVar.Name.c_str(), simVar.ID, simVar.Value, ival, simVar.StringValue.c_str());
#endif
}
